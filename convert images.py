...
imageName = "ok" #no file ending, has to be .jpg
...
count = 1
totalMessage = ""
image = cv2.imread(f"./{imageName}.jpg")
resized = cv2.resize(image,(imageWidth,imageHeight))
cv2.imwrite(f"./imageConversion.jpg", resized)  
totalMessage += turnascii(f"./imageConversion.jpg")
print(len(totalMessage))
print(f"Rendered image")
 
with open(f"{outputPath}{imageName} (img).txt", "w+", encoding="utf-8") as fle:
